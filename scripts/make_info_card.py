...
ROWS = [
    ...
    ("kv", "Name", "Vishesh Kaushal"),
    ("kv", "Role", "Software Engineer"),
    ("kv", "College", "NSUT (CSAI)"),
    ("kv", "Year", "3rd Year B.Tech"),
    ("gap",),

    ("sec", "Stack"),
    ("kv", "Languages", "Java, Python, C++"),
    ("kv", "Backend", "Spring Boot, SQL"),
    ("kv", "Frontend", "React, HTML, CSS"),
    ("kv", "Tools", "Git, Linux, Docker"),
    ("gap",),

    ("sec", "Current"),
    ("bul", "Grinding NeetCode 250"),
    ("bul", "Competitive Programming"),
    ("bul", "Building AI & Backend Projects"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">vishesh@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">vishesh</tspan><tspan fill="{MUTED}">@</tspan>'
